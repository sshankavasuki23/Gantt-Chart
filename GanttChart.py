...
if 'data' not in st.session_state:
    st.session_state.data = pd.DataFrame([
        ...
        {"Row Type": "Task", "Group": "Pub 4 - Thermal model", "Activity description": "Model Design", "Person incharge": "Sathya", "Label": "PUB1", "Start month": 1, "End month": 5, "Color": "Orange", "Alignment": "Left", "Bold": False, "Italics": True, "Font size": 10},
    ])
...
def draw_scientific_gantt(data, start_dt, num_months):
    ...
    fig, ax = plt.subplots(figsize=(24, (len(data) + H_TOTAL + 0.5) * ROW_H))
    ...
    ax.text((ACT_W + STAFF_W)/2, len(data) + (H_TOTAL/2), "Activities", ha='center', va='center', fontweight='bold', fontsize=18)
    ...
    for yr in sorted(list(set(year_map))):
        ...
        ax.text(curr_x + span/2, len(data) + N_H + M_H + (Y_H/2), str(yr), ha='center', va='center', fontweight='bold', fontsize=13)
        curr_x += span

    for i in range(num_months):
        x = ACT_W + STAFF_W + i
        ax.add_patch(patches.Rectangle((x, len(data) + M_H), 1, N_H, facecolor='white', edgecolor='black', lw=1))
        ax.text(x + 0.5, len(data) + M_H + (N_H/2), str(i + 1), ha='center', va='center', fontsize=10)
    ...
    for idx, row in data.iterrows():
        ...
        if row_type == 'Header':
            ...
            ax.add_patch(patches.Rectangle((0, y), ACT_W + STAFF_W + num_months, 1, facecolor='#d3d3d3', edgecolor='black', lw=1, alpha=0.4))
            ax.text((ACT_W + STAFF_W + num_months) * 0.5, y + 0.5, str(row.get('Activity description') or "").upper(), ha='center', va='center', fontweight=f_weight, fontstyle=f_style, fontsize=f_size)
        else:
    ...
    # --- Merged Activity Cell Overlay ---
    i = 0
    while i < len(data):
        row = data.iloc[i]
        group_val = str(row.get('Group') or "").strip()
        if group_val and str(row.get('Row Type')) == 'Task':
            j = i + 1
            while j < len(data) and str(data.iloc[j].get('Group') or "").strip() == group_val and str(data.iloc[j].get('Row Type')) == 'Task':
                j += 1
            if j - i > 1:
                span = j - i
                y_bottom = len(data) - j
...
with st.container(border=True):
    ...
    plt.close(final_fig) # Clear memory
